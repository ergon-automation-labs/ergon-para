pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-para"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/para_bot/bin/para_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyPara.Release.create()'"
          $RELEASE_BIN eval 'BotArmyPara.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyPara.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyPara.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_para/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_para/releases/start_erl.data)
        fi
